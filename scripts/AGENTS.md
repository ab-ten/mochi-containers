# Scripts Guidelines

- scripts/pre-deploy-check.sh と docs/pre-deploy-check.md の内容は同期させること
- scripts/deploy-service.sh と docs/deploy-service.md の内容は同期させること
- scripts/deploy-vars.subr と docs/deploy-vars.subr.md の内容は同期させること
- scripts/print_unshare_id.sh と docs/print_unshare_id.md の内容は同期させること
- scripts/container-build.sh と docs/container-build.md の内容は同期させること
- scripts/generate-index-html.sh と docs/generate-index-html.md の内容は同期させること
